...
bool BNO080::writeFRSRecord(uint16_t recordID, uint32_t *data, uint8_t lengthWords) {
	if (lengthWords > 15)
		return false; // Ограничение длины

	shtpData[0] = 0xFD; // FRS Write Request
	shtpData[1] = 0x00; // Reserved
	shtpData[2] = recordID & 0xFF;
	shtpData[3] = (recordID >> 8) & 0xFF;
	shtpData[4] = lengthWords; // Number of words
	shtpData[5] = 0x00;				 // Word Offset
	shtpData[6] = 0x00;				 // Reserved
	shtpData[7] = 0x00;				 // Reserved

	for (uint8_t i = 0; i < lengthWords; i++)
	{
		shtpData[8 + i * 4 + 0] = data[i] & 0xFF;
		shtpData[8 + i * 4 + 1] = (data[i] >> 8) & 0xFF;
		shtpData[8 + i * 4 + 2] = (data[i] >> 16) & 0xFF;
		shtpData[8 + i * 4 + 3] = (data[i] >> 24) & 0xFF;
	}

	uint8_t packetLength = 8 + lengthWords * 4;

	return sendPacket(CHANNEL_CONTROL, packetLength);
}